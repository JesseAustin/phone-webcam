...
void HandshakeServer::listenLoop()
{
    ...
    while (running_) {
        ...
        long long ts = jsonGetLong(msg, "ts");
        if (ts > 0) {
            long long now = (long long)std::time(nullptr);
            long long diff = now - ts;
            if (diff < -5 || diff > 30) {  // allow 5s clock skew, reject >30s stale
                blog(LOG_WARNING, "HandshakeServer: rejected %s — timestamp stale (%llds)", phoneIp, diff);
                continue;
            }
        }
