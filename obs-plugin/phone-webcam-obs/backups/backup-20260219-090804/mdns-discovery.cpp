...
void MdnsDiscovery::stop()
{
    ...
    if (discovery_thread_.joinable()) {
        ...
        size_t tid_target = std::hash<std::thread::id>{}(discovery_thread_.get_id());
        blog(LOG_INFO, "mDNS: discovery_thread join check this=%zu target=%zu joinable=%d",
             tid_this, tid_target, (int)discovery_thread_.joinable());

        if (discovery_thread_.get_id() != std::this_thread::get_id()) {
...
void MdnsDiscovery::discoveryLoop()
{
    ...
    while (running_) {
        ...
        for (int rr = 0; rr < totalRR && offset < (size_t)received; rr++) {
            ...
            uint16_t rdLen = (buf[offset] << 8) | buf[offset + 1];
            offset += 2;

            if (rrType == 33) { // SRV
                if (offset + 6 <= (size_t)received) {
                    port = (buf[offset + 4] << 8) | buf[offset + 5];
                    size_t nameOffset = offset + 6;
                    targetHost = readDnsName(buf, received, nameOffset);
                    blog(LOG_INFO, "mDNS: SRV record - host=%s port=%d",
        ...
        if (port > 0) {
            auto now = std::chrono::steady_clock::now();
            constexpr auto DEBOUNCE_MS = 1000;

            bool sameService = (targetHost == last_service_hostname_ && port == last_service_port_);

            // If this is the same SRV we've already accepted, ignore duplicates.
            if (sameService) {
                continue;
            }
