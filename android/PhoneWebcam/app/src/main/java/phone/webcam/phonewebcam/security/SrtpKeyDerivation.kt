...
object SrtpKeyDerivation {
    ...
    fun deriveKeys(password: String, salt: ByteArray): Pair<ByteArray, ByteArray> {
        ...
        // 30 bytes = 240 bits
        val spec: KeySpec = PBEKeySpec(password.toCharArray(), salt, 1000, 240)
        ...
        Log.d("SrtpKeys", "key=${masterKey.joinToString("") { "%02x".format(it) }}")
        Log.d("SrtpKeys", "salt=${masterSalt.joinToString("") { "%02x".format(it) }}")
